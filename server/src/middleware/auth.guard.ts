/**
 * Middleware (Guard) для защиты маршрутов (эндпоинтов) от неавторизованного доступа.
 * Проверяет наличие и валидность токенов (Bearer, Cookie), API ключей и прав доступа.
 * Также предоставляет декораторы для извлечения данных авторизованного пользователя из запроса.
 */
import {
  CanActivate,
  ExecutionContext,
