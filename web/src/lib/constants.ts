/**
 * Глобальные константы приложения (Constants).
 * Здесь хранятся неизменяемые значения, перечисления (Enums) и словари,
 * такие как поддерживаемые языки, типы проекций, форматы дат и 
 * ключи маршрутизации (query-параметры).
 */
