...
e2e-web-dev:
	cd e2e && PLAYWRIGHT_BASE_URL=http://127.0.0.1:2284 PLAYWRIGHT_DISABLE_WEBSERVER=1 pnpm exec playwright test --project=web

e2e-web-dev-ui:
	cd e2e && PLAYWRIGHT_BASE_URL=http://127.0.0.1:2284 PLAYWRIGHT_DISABLE_WEBSERVER=1 pnpm exec playwright test --ui --project=web

e2e-api-dev:
	cd e2e && PLAYWRIGHT_BASE_URL=http://127.0.0.1:2284 PLAYWRIGHT_DISABLE_WEBSERVER=1 pnpm test

e2e-integration-dev:
	cd e2e && PLAYWRIGHT_BASE_URL=http://127.0.0.1:2284 PLAYWRIGHT_DISABLE_WEBSERVER=1 pnpm exec playwright test --project=integration

e2e-integration-dev-ui:
	cd e2e && PLAYWRIGHT_BASE_URL=http://127.0.0.1:2284 PLAYWRIGHT_DISABLE_WEBSERVER=1 pnpm exec playwright test --ui --project=integration
...
test-medium-dev:
	docker exec -it immich_server_dev /bin/sh -c "pnpm run test:medium"
